...
void ThreadIRCSeed2()
{
	...
	int ntimeout = 1000;
        ...
        if (TestNet())
        {
            Send(hSocket, "JOIN #spectorcoinTEST\r");
            Send(hSocket, "WHO #spectorcoinTEST\r");
        } else
        {
	    LogPrintf("JOIN #spectorcoin\n");
            // randomly join #spectorcoin00-#spectorcoin05
            //int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #spectorcoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #spectorcoin%02d\r", channel_number).c_str());
        };
