...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeSpecTorAmount"))
        settings.setValue("nAnonymizeSpecTorAmount", 1000);

    nDarksendRounds = settings.value("nDarksendRounds").toLongLong();
    nAnonymizeSpecTorAmount = settings.value("nAnonymizeSpecTorAmount").toLongLong();
    ...
    if (settings.contains("nAnonymizeSpecTorAmount"))
        SoftSetArg("-anonymizeSpecToramount", settings.value("nAnonymizeSpecTorAmount").toString().toStdString());
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case anonymizeSpecTorAmount:
            nAnonymizeSpecTorAmount = value.toInt();
            settings.setValue("nAnonymizeSpecTorAmount", nAnonymizeSpecTorAmount);
            emit anonymizeSpecTorAmountChanged(nAnonymizeSpecTorAmount);
            break;
#ifdef USE_NATIVE_I2P
        case I2PUseI2POnly:
        {
