...
unsigned int nStakeMaxAge = 16 * 24 * 60 * 60; // 16 Days
...
const string strMessageMagic = "SpecTor Signed Message:\n";
...
int64_t GetProofOfWorkReward(int nHeight, int64_t nFees)
{
    ...
    else if (pindexBest->nHeight < 210)
    {
        nSubsidy = 3000000 * 10 * COIN;
    }
...
bool SendMessages(CNode* pto, bool fSendTrickle)
{
    ...
    if (lockMain) {
        ...
        if (pto->nPingNonceSent == 0 && pto->nPingUsecStart + PING_INTERVAL * 10000 < GetTimeMicros()) {
