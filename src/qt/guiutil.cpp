...
{
    // Convert spector:// to spector:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("spector://"))
    {
        uri.replace(0, 12, "spector:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "SpecTor.lnk";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart)
    {
        ...
        // Get a pointer to the ISpecTorLink interface.
        IShellLink* psl = NULL;
        ...
        if (SUCCEEDED(hres))
        {
            ...
            // Query ISpecTorLink for the IPersistFile interface for
...
{
    return GetAutostartDir() / "spector.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("SpecTor-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  spector-qt [" + tr("command-line options") + "]                     " + "\n";
